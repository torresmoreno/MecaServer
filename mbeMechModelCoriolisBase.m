classdef mbeMechModelCoriolisBase < mbeMechModelBase
    ...
    properties(GetAccess=public,SetAccess=public)
        ...
        q_init_aprox=zeros(mbeMechModelCoriolis.dep_coords_count,1);
    ...
    methods(Access=public)
        ...
        function phiq = jacob_phi_q(me,q)
            ...
            phiq(1,:) = [2*(x1-me.xA) 2*(y1-me.yA) 0 0 0 0 ];
            phiq(2,:) = [0 0 2*(x2-me.xB) 2*(y2-me.yB) 0 0 ];                        
            phiq(3,:) = [-(y2-me.yB) (x2-me.xB) (y1-me.yB) -(x1-me.xB) 0 0];            
            phiq(4,:) = mbe_iff(abs(sin(theta)) < 0.7,... 
                            [0,             1,             0,             0, -LA1*cos(theta), 0], ...
                            [1,             0,             0,             0,  LA1*sin(theta), 0]);
            
            phiq(5,:) = mbe_iff(abs(sin(beta)) < 0.7,... 
                            [0,             0,             0,             1, 0, -L2B*cos(beta)], ...
                            [0,             0,             1,             0, 0, L2B*sin(beta)]);                         
        %disp(phiq)
        end % jacob_phi_q()
        ...
        function phiqpqp = jacob_phiqp_times_qp(me,q,qp)
            ...
            dotphiq(1,:) = [2*(x1p-0) 2*(y1p-0) 0 0 0 0];
            dotphiq(2,:) = [0 0 2*(x2p-0) 2*(y2p-0) 0 0];                        
            dotphiq(3,:) = [-(y2p-0) (x2p-0) (y1p-0) -(x1p-0) 0 0];            
            ...
            dotphiq(5,:) = mbe_iff(abs(sin(beta)) < 0.7,... 
                            [0,             0,             0,             0, 0, L2B*sin(beta)*betap], ...
                            [0,             0,             0,             0, 0, L2B*cos(beta)*betap]);                                               
