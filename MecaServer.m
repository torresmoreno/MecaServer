function varargout = MecaServer(varargin)
...
axes(handles.axes1)
dibuja(mbs.mechanism_type,[q;mbs.fixed_points'],qp);
...
function []=dibuja(tipo,q,varargin)
switch tipo
...
if (isappdata(new_hAx,'hMechPlot'))
    mpSt = getappdata(new_hAx,'hMechPlot');
...
if (isappdata(gca,'hMechPlot'))
    mpSt = getappdata(gca,'hMechPlot');
    % Reutilizar el objeto y solo actualizar las coordenadas no fijas:
    mpSt.keep_axis_limits = 1;
    mpSt.skip_drawnow=1;
    mpSt.plot(q);
